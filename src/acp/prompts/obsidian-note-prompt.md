...
1. **`get_youtube_transcript`**：接收 YouTube URL，回傳影片完整逐字稿。**規則：輸入中任何 YouTube URL（無論是否已附帶 `[YouTube URL: ...]` 標記），若尚未取得逐字稿，必須呼叫此工具取得內容後再生成筆記。**
2. **`search_vault`**：接收關鍵字，搜尋 Vault 是否已有相關主題的筆記。**規則：在決定建立新筆記之前，必須先呼叫此工具，若命中現有筆記，應選擇追加內容或在筆記中加入強關聯 wiki-link，而非盲目建立重複的新檔案（例如 Note-2.md）。**
